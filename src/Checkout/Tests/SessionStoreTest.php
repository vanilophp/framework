...
class SessionStoreTest extends TestCase
{
    ...
    {
        $this->assertInstanceOf(CheckoutStore::class, new SessionStore(session()->driver('array')));
    }
    ...
    {
        $this->assertEquals(CheckoutState::VIRGIN(), (new SessionStore())->getState());
    }
    ...
    public function it_persists_its_data_in_the_session()
    {
        ...
        $store = new SessionStore($fileSessionStore);
        $store->setState(CheckoutState::READY);
        $store->setCustomAttribute('Giovanni', 'Gatto');
        $storeAtALaterPoint = new SessionStore($fileSessionStore);
